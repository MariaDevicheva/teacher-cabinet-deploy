...
const DEMO_PENDING_APPLICATIONS = [
  { id: 101, studentName: 'Иванов Иван Иванович', group: 'УИБО-01-22', date: '25.02.2026', gpa: 4.8, course: 3, degree: 'Бакалавриат', topicProposed: false },
  { id: 102, studentName: 'Петров Пётр Петрович', group: 'УИБО-02-23', date: '27.02.2026', gpa: 4.5, course: 4, degree: 'Бакалавриат', topicProposed: true, proposedTopic: 'Анализ больших данных' },
];
...
function TeacherCabinet({ user, profile, onLogout }) {
  ...
  <h2 className={`font-semibold text-lg flex items-center ${theme.text}`}>
    ...
    Добрый день, {profile.full_name.split(' ').slice(1).join(' ')}!
      ...
      <div className="flex gap-2">
        <button onClick={() => setActiveNewsTab('all')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all hover:scale-105 ${activeNewsTab === 'all' ? (isDark ? 'bg-[#A78BFA] text-white' : 'bg-[#3B82F6] text-white') : (isDark ? 'bg-[#1E1E2A] text-gray-400' : 'bg-gray-100 text-gray-500')}`}>Все</button>
<button onClick={() => setActiveNewsTab('important')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all hover:scale-105 ${activeNewsTab === 'important' ? (isDark ? 'bg-[#A78BFA] text-white' : 'bg-[#3B82F6] text-white') : (isDark ? 'bg-[#1E1E2A] text-gray-400' : 'bg-gray-100 text-gray-500')}`}>Важное</button>
      </div>
  ...
  // Проверяем, сдан ли предыдущий раздел
  let previousNotCompleted = false;
  if (idx > 0) {
    const prevTask = s.submittedTasks.find(x => x.taskIndex === idx - 1);
    // Предыдущий не сдан, если его нет или он не оценен
    previousNotCompleted = !prevTask || prevTask.gradeStatus !== 'graded' || parseFloat(prevTask.grade) < 3;
  }
  
  // Если предыдущий не сдан, этот раздел не может быть "на проверке"
  const effectiveStatus = previousNotCompleted && t ? 'locked' : (t ? t.gradeStatus : 'not_submitted');
  const isGraded = effectiveStatus === 'graded';
  const isSubmitted = effectiveStatus !== 'graded' && effectiveStatus !== 'not_submitted' && effectiveStatus !== 'locked';
  const isNotSubmitted = effectiveStatus === 'not_submitted';
    ...
    <div key={idx} className={`rounded-lg overflow-hidden ${theme.cardInner}`}>
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm ${isGraded || isSubmitted ? theme.text : theme.textMuted}`}>
              {name}
            </span>
          </div>
          <div className="flex items-center gap-2">
            {isGraded ? (
              <span className={`text-xs px-2 py-1 rounded-full ${isDark ? 'bg-green-600/20 text-green-400' : 'bg-green-100 text-green-700'}`}>
                Оценка: {t.grade}
              </span>
            ) : isSubmitted ? (
              <span className={`text-xs px-2 py-1 rounded-full ${isDark ? 'bg-yellow-600/20 text-yellow-400' : 'bg-yellow-100 text-yellow-700'}`}>
        ...
        {/* Показываем дату сдачи только если задание реально сдано и не заблокировано */}
        {t && !isLocked && (
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-3 border-t" style={{ borderColor: isDark ? '#2A2A3A' : '#E5E7EB' }}>
          {t && !isLocked ? (
            // ===== СДАННОЕ ЗАДАНИЕ (не заблокированное) =====
            <>
              <div className="pt-3">
            ...
            // ===== НЕСДАННОЕ ИЛИ ЗАБЛОКИРОВАННОЕ ЗАДАНИЕ =====
            <div className="pt-3">
              <div className="mb-3">
                <p className={`text-xs font-medium ${theme.textMuted}`}>Описание:</p>
